...
class GreedySearch():
    ...
    def optimize(self, solution: Solution) -> Solution:
        logger.debug(f"[Optimize] before optimizing solution =>\n{solution}")
        solution = self.local_search(solution)
        logger.debug(f"[Optimize] After local_search =>\n{solution}")
        solution = self.optimize_depots(solution)
        logger.debug(f"[Optimize] After optimize_depots =>\n{solution}")
        logger.debug(f"[DEBUG] Before set_tour_length - Tours after optimize_depots: {[[node.id for node in tour] for tour in solution.tours]}")
        solution.set_tour_length(self.problem.calculate_tour_length(solution))
        logger.debug(f"[DEBUG] after set_tour_length - Tours after optimize_depots: {[[node.id for node in tour] for tour in solution.tours]}")
    ...
    def optimize_depots(self, solution: Solution) -> Solution:
        ...
        logger.debug(f"New Tours: {[[n.id for n in subr] for subr in new_tours]}")
    ...
    def _insert_depot_with_min_delta(self, segment):
        ...
        """
        logger.debug(f"Delta L 삽입 전 segment: {[n.id for n in segment]}")
        ...
        if best_insertion_index > 0:
            ...
            logger.debug(f"Depot 삽입 인덱스={best_insertion_index}, Delta L={min_delta}, "
                        f"Segment={ [n.id for n in segment]}")
