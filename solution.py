...
class Solution():
    ...
    def __repr__(self) -> str:
        if self.tour_length < np.inf:
            presentation = f"Total tour length (with depot): {self.tour_length}\n"
        else:
            presentation = "Tour length: (not calculated)\n"

        # Add individual tour details
        for i, tour in enumerate(self.tours):
            tour_distance = sum(
                tour[j].distance(tour[j + 1]) for j in range(len(tour) - 1)
            )
            presentation += (
                f"Tour {i} (excluding depot): "
                + " -> ".join(str(node.id) for node in tour)
                + f" | Length: {tour_distance:.4f}\n"
            )
