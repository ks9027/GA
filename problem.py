...
class Problem(): #문제 데이터를 읽어와서 problem 객체로 초기화
    ...
    def plot(self, solution=None, path=None):
        ...
        for i, node in enumerate(self.nodes):
            if node.is_customer():
                # Customer 노드: 파란색 원
                ax.scatter(node.x, node.y, c='blue', s=15)
                # Customer 번호와 Demand 표시
                ax.text(node.x, node.y, f"Node {i + 1}\nD:{node.get_demand():.2f}", fontsize=5, ha='center', va='bottom')

            elif node.is_depot():
                # Depot: 빨간색 원
                ax.scatter(node.x, node.y, c='red', s=80)
        ...
        plt.legend(by_label.values(), by_label.keys(),
                loc='upper right',
                prop={'size': 6})
        total_distance = 0.0  # 전체 경로 길이를 저장
        # 솔루션 플롯
        if solution is not None:
            # 여러 색상을 위한 색상 리스트 및 초기화
            ...
            for tour in solution.get_tours():
                # 각 sub-tour의 시작과 끝에 depot 추가
                full_tour = [self.get_depot()] + tour + [self.get_depot()]

                # 경로 시각화 (각 sub-tour를 다른 색으로 표시)
                for i in range(len(full_tour) - 1):
                    ...
                    total_distance += first_node.distance(second_node)  # 총 거리 계산
                # 다음 sub-tour를 위해 색상 변경
                current_color_index += 1
        plt.legend([f"Total Distance: {total_distance:.3f}"], loc='upper left', fontsize=8)
