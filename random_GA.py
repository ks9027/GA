...
from random import shuffle, randint, uniform, choice  # 주의: random() 함수는 여기서 포함되지 않음
...
class random_GA():
            ...
            while len(new_population) < self.population_size:
                ...
                if random.random() < self.crossover_prob:
                    ...
                    if random.random() < self.mutation_prob:
                        ...
                        if random.random() < (0.2 + (0.6 - 0.2) * (1 - alpha)):
    ...
    def _initial_population(self) -> List[Solution]:
        ...
        cust_ids = self.problem.get_customer_ids()  # depot 제외 고객 노드 ID 리스트
        #logger.debug(f"[DEBUG] Initial customer IDs (Before shuffle): {cust_ids}")

        for i in range(self.population_size):
            shuffled = cust_ids[:]  # 원본 유지
            random.shuffle(shuffled)  # 고객 노드 순서 랜덤 섞기
            #logger.debug(f"[DEBUG] Shuffled customer IDs: {shuffled}")

            # 섞인 순서대로 노드 객체 리스트 만들기
            shuffled_nodes = [self.problem.get_node_from_id(cid) for cid in shuffled]
            #logger.debug(f"[DEBUG] Shuffled customer nodes (IDs): {[node.id for node in shuffled_nodes]}")

            raw_route = [depot] + shuffled_nodes.copy() + [depot]

            # 2. 고객 수의 절반만큼 depot을 무작위 위치에 추가 삽입
            num_customers = len(shuffled_nodes)
            num_extra_depots = num_customers // 2  # 내림
            # 중간 위치(시작 depot과 마지막 depot 제외, 인덱스 1 ~ num_customers)를 무작위 선택
            insert_positions = sorted(random.sample(range(1, num_customers), num_extra_depots), reverse=True)

            for pos in insert_positions:
                # raw_route는 리스트이므로 insert() 사용 가능
                raw_route.insert(pos + 1, depot)

            # 3. 이제 raw_route를 이용해 Solution 객체를 생성합니다.
            raw_sol = Solution(tours=[raw_route])

            # Greedy 최적화 적용
            opt_sol = self.gs.optimize(raw_sol)
            #logger.debug(f"[DEBUG] Optimized solution after optimization - Tours: {[[node.id for node in tour] for tour in opt_sol.tours]}")

            # 여기서 optimize_depots를 한 번만 호출합니다.
            opt_sol = self.gs.optimize_depots(opt_sol)
            #logger.debug(f"[DEBUG] Forced optimize_depots() - Tours: {[[node.id for node in tour] for tour in opt_sol.tours]}")

            opt_sol.set_tour_index()
            #logger.debug(f"[DEBUG] After setting new tours - Tours: {[[node.id for node in tour] for tour in opt_sol.tours]}")

            initial_pop.append(opt_sol)

        #logger.debug(f"[DEBUG] Initial population created: {[len(sol.tours) for sol in initial_pop]} solutions")
        return initial_pop
